...
class ImageToPDFConverter:
    ...
    def initialize_ui(self):
        title_label = tk.Label(self.root, text='convert Image to pdf', font=('Helvetica', 16, "bold"))
        ...
        label =tk.Label(self.root, text="Enter the PDF name as:")
        label.pack()
        
        pdf_name_entry =tk.Entry(self.root, textvariable=self.output_pdf_name, width=40, justify='center')
        pdf_name_entry.pack()
        
        convert_button = tk.Button(self.root, text='convert to pdf NOW', command=self.convert_images_to_pdf)
        convert_button.pack(pady=(20, 40))
        
    def  select_images(self):
        self.image_paths = filedialog.askopenfilenames(title='select images', filetypes=[("image file", "*.png;*.jpg;*.jpeg")])
    ...
    def convert_images_to_pdf(self):
        ...
        for image_path in self.image_paths:
            ...
            y_centred = (792 - new_heigth) /2
            
            pdf.setFillColorRGB(255, 255, 255)
            pdf.rect(0, 0, 612, 792, fill=True)
            pdf.drawInlineImage(img, x_centered, y_centred, width=new_width, height=new_heigth)
            pdf.showPage()
        pdf.save()
